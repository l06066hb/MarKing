...
`WikiLink Backlinks` · `Knowledge Graph` · `Command Palette` · `Focus Mode` · `MathLive Formulas` · `Excalidraw` · `DOCX/PDF/HTML Export`
...
- **Truly local-first** — Plain `.md` files on your disk. No cloud lock-in, no proprietary formats, no telemetry
- **Native performance** — Natively compiled, < 2 s cold start, smooth scrolling through 100K-word docs
- **Knowledge management** — WikiLink backlinks + interactive knowledge graph turn isolated notes into a connected web
- **Immersive writing** — Focus Mode, Command Palette, Breadcrumb nav for a distraction-free writing environment
- **Professional content** — MathLive formulas, Excalidraw whiteboard, Mermaid diagrams — one-stop technical authoring
- **One-click export** — DOCX template engine v2.0 / PDF / HTML — from academic papers to business reports
- **Cross-platform** — Windows · macOS (Intel & Apple Silicon) · Linux, one consistent experience
...
*   **Full-Featured Professional Editor**  
    Handles 100K+ word documents with smooth scrolling. Ships with 70+ Markdown smart snippets and contextual suggestions for a professional editing experience.
...
*   **Bi-directional Links & Knowledge Graph** `NEW`  
    Full `[[WikiLink]]` backlink system paired with an interactive knowledge graph. Force-directed layout, zoom & pan, 10K+ node smooth interaction, graph export (PNG/SVG), and immersive fullscreen.

    <img src="docs/marking-knowledge-graph-en.png" alt="MarKing Knowledge Graph - Backlink Visualization" width="800">

*   **Command Palette & Quick Switcher** `NEW`  
    `Ctrl+Shift+P` for a global command search panel; `Ctrl+P` for instant fuzzy file-name navigation. Breadcrumb bar above the editor shows the real-time directory path.

    <img src="docs/marking-command-palette-en.png" alt="MarKing Command Palette" width="800">

*   **Focus Mode** `NEW`  
    Typewriter Mode (current line always centered) + Highlight Mode (dims non-active paragraphs) for distraction-free immersive writing.

*   **Callout Blocks** `NEW`  
    Renders `> [!NOTE]`, `> [!WARNING]` and other admonition types in real-time, compatible with both GitHub Alerts and Obsidian Callout syntax.

### 3. Creation & Interaction

*   **MathLive Formula Editor**  
    Integrated WYSIWYG math formula editor supporting inline and block equations. Comes with a virtual math keyboard, fully offline rendering — no manual LaTeX needed.
...
**If MarKing has improved your writing workflow, please give it a Star**

It's the most direct encouragement for an independent developer — and helps others discover this tool.
...
Sustaining a purely local-first, zero-tracking, ad-free cross-platform app is no small feat.  
If you value MarKing's quality, consider becoming an early sponsor:
